...
object ExecutorHelper {
    private const val TAG: String = "ExecutorHelper"
    private var isPaused: Boolean = false
    private var executorHelper: ThreadPoolExecutor
    ...
    init {
        ...
        val threadFactory = ThreadFactory {
            ...
            thread.name = "ExecutorHelper-worker-" + seq.getAndIncrement()
            return@ThreadFactory thread
        }

        executorHelper = object : ThreadPoolExecutor(
            corePoolSize,
            maxPoolSize,
            keepAliveTime,
            unit,
    ...
    fun execute(@IntRange(from = 0, to = 10) priority: Int = 0, runnable: Runnable) {
        executorHelper.execute(PriorityRunnable(priority, runnable))
    }
