        ...
        viewModelScopeCoroutine(viewModelScope) {
                doWork { 
                	CoroutineDataFetcher { apiService.config2(page) }.startFetchData(1,
                		"cache//com.liyihuanx.module_common.service.ConfigService//config2?page=${page}",
                		1800000
                	).collect {
                		onResult.invoke(it) 
                	} 
        ...
        viewModelScopeCoroutine(viewModelScope) {
                doWork { 
                	CoroutineDataFetcher { apiService.getData() }.startFetchData(cacheStrategy,
                 		"cache//com.liyihuanx.module_common.service.ConfigService//getData?cacheStrategy=${cacheStrategy}",
                		1800000
                ).collect {
                		onResult.invoke(it) 
                	} 
